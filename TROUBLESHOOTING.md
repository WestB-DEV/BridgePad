...
- **`Timeout: BridgePad did not acknowledge the command` after pairing:** Install a matching `0.1.4-test.1` or later APK and FAP. Earlier firmware could send the `HELLO` status and acknowledgement too close together, causing the acknowledgement to be dropped while the first BLE indication was still in flight.
...
- **Text is rejected:** V1 permits printable US-QWERTY ASCII only; emoji and accented characters are blocked. The live typing field translates the Android keyboard's Enter key into USB Enter; use the separate Enter control when composing reviewed text.
